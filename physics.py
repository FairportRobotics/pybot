...
class PhysicsEngine:
    def __init__(self, physics_controller: PhysicsInterface, robot: "MyRobot"):
        ...
        self.drivesim = wpisim.DifferentialDrivetrainSim(
            self.system,
            inchesToMeters(constants.Robot.DISTANCE_BETWEEN_WHEELS_INCHES), # Track width (meters)
            plant.DCMotor.CIM(2),                                           # 2 CIM motors per side
            constants.Robot.GEAR_RATIO,                                     # Gearing (Toughbox Mini)
            inchesToMeters(constants.Robot.WHEEL_RADIUS_INCHES)             # Wheel radius (meters)
        )
    ...
    def update_sim(self, now: float, tm_diff: float) -> None:
        ...
        counts_per_m = (1 / (2 * math.pi * inchesToMeters(constants.Robot.WHEEL_RADIUS_INCHES)) * constants.Robot.GEAR_RATIO * constants.Robot.ENCODER_TICKS_PER_ROTATION
